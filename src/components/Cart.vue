<template>
<div>
  <section class="cart py-16">
    
        <div class="order container mx-auto xl:w-1/2" v-if='items'>
            <div class="flex items-center border-b border-gray-300 pb-4">
                <img src="/img/cart-black.png" alt="">
                <h1 class="font-bold ml-4 text-2xl">Order summary</h1>
            </div>
            <div class="pizza-list">
                
                    <div class="flex items-center my-8" v-for="(item, index) in items" :key="index">
						                      <div class="flex items-center my-8">
						    <img class="w-24" :src="item.item.image" alt="">
						    <div class="flex-1 ml-4">
						        <h1>{{item.item.name}}</h1>
						        <span>{{item.item.category.name}}</span>
						    </div>
						    <span class="flex-1">{{item.qty}} Pcs</span>
						    <span class="font-bold text-lg"> $ {{item.qty*item.item.price}}</span>
						</div>

                    </div>
                

            </div>
            <hr>
            <div class="text-right py-4">
                <div>
                    <span class="text-lg font-bold">Total Amount: </span>
                    <span class="amount text-2xl font-bold ml-2">$ {{totalPrice}}</span>
                </div>
                    <div v-if='user'>
                    	<span class="text-red-500 text-sm" v-if='error'>
		                  <ul v-if='error' class="text-danger">
		                  <li>{{validation}} </li>
		                  <li v-for='(er,errorind) in error' v-bind:key='errorind'>{{errorind}} : {{er[0]}}</li>
		                </ul>
		                </span>
                        <form @submit.prevent="orderNow" method="POST" class="mt-12" id="payment-form">
                            <div class="relative w-1/2 ml-auto mb-4">
                                <select id="paymentType" name="paymentType" class="block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded leading-tight focus:outline-none focus:shadow-outline">
                                    <option value="cod">Cash on delivery</option>
                                    <option value="card">Pay with card</option>
                                </select>
                                <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                                    <svg class="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/></svg>
                                </div>
                            </div>
                            <input name="phone" v-model='phone' class="border border-gray-400 p-2 w-1/2 mb-4" type="text"
                                   placeholder="Phone number">
                            <input name="address" v-model='address' class="border border-gray-400 p-2 w-1/2" type="text" placeholder="Address">
                            <div class="mt-4">
                                <div id="card-element"></div>
                                <button type="submit" class="btn-primary px-6 py-2 rounded-full text-white font-bold mt-6">Order
                                    Now</button>
                            </div>
                        </form>
                    </div>
                    <div v-else>
                    <router-link to='/login'
                       class="inline-block cursor-pointer btn-primary px-6 py-2 rounded-full text-white font-bold mt-6">Login
                        to continue</router-link >
                    </div>    
               
            </div>
        </div>
    
        <div class="empty-cart py-16" v-else>
            <div class="container mx-auto text-center">
                <h1 class="text-3xl font-bold mb-2">Cart Empty 😕</h1>
                <p class="text-gray-500 text-lg mb-12">You probably haven't ordered a pizza yet. <br>
                    To order a pizza, go to the main page.</p>
                <img class="w-2/5 mx-auto" src="/img/empty-cart.png" alt="empty-cart">
                <router-link  to="/" class="inline-block px-6 py-2 rounded-full btn-primary text-white font-bold mt-12">Go back</router-link >
            </div>
        </div>
</section>
   </div> 
</template>
<script>
import axios from "axios";
export default {
  name: "Cart",
  data() {
    return {
      items:null,
      totalPrice:0,
      error:null,
      showModal:false,
      user:null,
      phone:'',
      address:'',
      validation:'',
    };
  },
  created() {
  	if (localStorage.getItem('user_info') !=null) {
  		this.user=JSON.parse(localStorage.getItem('user_info'));
  	}
  	if (localStorage.getItem('product_cart') !=null) {
  		let cart = JSON.parse(localStorage.getItem('product_cart'));
  		this.items = cart.items;
  		this.totalPrice=cart.totalPrice;
  	}
  
  },
  methods: {
   async orderNow() {

	  let data={user_id:this.user.id,address:this.address,phone:this.phone,items:JSON.stringify(this.items),token:localStorage.getItem('user_token')};
    const response = await axios.post('order/create',data);
    if (response.data.status==422) {
        this.validation =response.data.message;
        this.error=response.data.data;
       }else{
       	this.error=null;
        localStorage.removeItem('product_cart');
        localStorage.removeItem('totalQty');
        this.items = null;
  		this.totalPrice=0;
  		document.getElementById('cartCounter').innerHTML =0;
        alert('Order placed');
        this.$router.push('orders');
       }

    }
  }
};
</script>